    ...
    <Card>
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              ...
              <div className="grid gap-3">
                <Input placeholder="Titre de la tâche" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
                ...
                <Button onClick={handleCreate} disabled={!form.title || createTodo.isPending}>
                  {createTodo.isPending && <Loader2 className="h-4 w-4 animate-spin mr-1" />}Créer
                </Button>
              </div>
            </DialogContent>
          </Dialog>
      ...
      <CardContent className="space-y-1.5">
        {isLoading ? (
          ...
          <p className="text-xs text-muted-foreground text-center py-3">Aucune tâche en cours</p>
        ) : (
          activeTodos.slice(0, 10).map(todo => (
            <div key={todo.id} className="flex items-center gap-2 p-2 rounded-md hover:bg-muted/50 group">
              <Checkbox
                checked={todo.status === "termine"}
                onCheckedChange={() => toggleStatus(todo)}
              />
              <div className="flex-1 min-w-0">
                <p className={`text-xs font-medium truncate ${todo.status === "termine" ? "line-through text-muted-foreground" : ""}`}>
                  {todo.title}
                </p>
                {todo.due_date && (
              ...
              <Badge variant="outline" className={`text-[8px] ${PRIORITY_COLORS[todo.priority] || ""}`}>
                {todo.priority}
              </Badge>
        ...
        {doneTodos.length > 0 && (
          <div className="pt-2 border-t">
            <p className="text-[10px] text-muted-foreground mb-1">Terminées récemment</p>
            {doneTodos.map(todo => (
